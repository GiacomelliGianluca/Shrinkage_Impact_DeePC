% Main script for an interpretable DeePC for PWA systems using an iHankel data-driven model
...
% string_data_hankel = 'Data_one_mode.mat';
...
lambda_g = 10; % two modes dataset
% lambda_g = 10^5; % single mode dataset
% Control Horizon
T_c = 50;
...
% clusters) (set to 1 for the experiment with a single mode and comment coherently the ensuing part)
n_m = 2;
...
% Hankel with the index of the data (SISO case)
index_H = building_Hankel(1:N_D, N_h, 1, 1, 0);

% Declaring the switching condition
cond.y = 0;
% Plotting the (i)Hankel matrix
next_fig_index = 4;
fig_map = Hankel_map_PWA(str_cl_type, n_m, H_label, plt_set, next_fig_index, 'Hankel');
% Plotting the data strcuture higlighting the missclassified points
% H_label_miss: matrix containing the labels of data and the
% misclassfication indicated as 0
[H_label_miss, fig_map_miss] = Hankel_map_miss_PWA(str_cl_type, fig_map, label, label_id, f_inv, H_label, index_H, N_ini, plt_set, 5, 'Hankel');
% Plotting the number and avarage magnitude (H_miss_mag_avg) of missclassified data for a column
% H_miss_num: number of misclassification
% H_miss_mag_avg_u: input average magnitude for each column 
% H_miss_mag_avg_y: output average magnitude for each column
[H_miss_num, H_miss_mag_avg_u, H_miss_mag_avg_y, fig_col_miss_num, fig_col_miss_mag_avg] = DS_miss_num_mag_PWA(str_cl_type, H_label_miss, H_u, H_y, plt_set, 6, 7);

% Identify the submatrices for each mode 
...
[~, sub_ind_H, sub_num, fig_H_sub] = Hankel_sub_PWA(str_cl_type, H_y, H_u, n_m, N_h, n_x, n_u, cond, H_label, fig_map, plt_set, 8);
...
[~, sub_ind_H_miss, sub_num_miss, fig_H_sub_miss] = Hankel_sub_miss_PWA(str_cl_type, H_label_miss, sub_ind_H, sub_num, H_u, N_h, n_x, n_u, fig_map_miss, plt_set, 9);
...
% Initializing cell
for i = 1 : (n_m + 1)
    % For every mode, in the cell is allocated a matrix corresponding with rows as the instants in T_c and the columns as hyperparameter
    % values. The last row of the cell corresponds to the effect of data columns of
    % multiple modes
...
for k= 1 : (N_ini + T_c)
    ...
    if k <= N_ini
     ...
     if k >= N_ini + 1          % Checking if the DeePC has to be executed
         ...
         % Solve the DeePC
         % iLasso-DeePC 
         [J_opt(fut), J_tr(fut), J_g(fut), g_opt(:,fut), u_opt_seq(:, fut), y_pre_seq(:, fut)] = DeePC_y_f_lambda_g(u_ini, y_ini, u_r_f(fut: fut + L - 1), y_r_f(fut: fut + L - 1), n_u, n_y, Q, R, N_h, N_ini, N_ex, H_u, H_y, N_D,  lambda_g);
         % % iCAP-DeePC 
         % [J_opt(fut), J_tr(fut), J_g(fut), g_opt(:,fut), u_opt_seq(fut,:), y_pre_seq(fut,:)] = CAP_DeePC_y_f_lambda_g(u_ini, y_ini, u_r_f(fut: fut + L - 1), y_r_f(fut: fut + L - 1), n_u, n_y, Q, R, N_h, N_ini, N_ex, H_u, H_y, N_D,  lambda_g, n_m, sub_ind_H, sub_num, slc_norm);
        ...
        if~isnan(y_pre_seq(:, fut))
            ...
            if sub_num > 0
                % BPI    
                BPI_singles = BPI_comp_h('id', g_opt(:,fut)', sub_ind_H, sub_num, n_x, n_u, n_m, Labels_id, fut, N_ini, f_inv);
                for f = 1 : (n_m+1)
...
fig_input = figure(10);
plot(steps, u_r_f(1:T_c),'k', 'linewidth',1), hold on,
plot(steps, u(N_ini+1:N_ini+T_c),'Color', [0.87 0.40 0.69],'linewidth',plt_set.thick), grid on,
xlim([0 49]);
str = str_xaxis;
...
plot(steps, y(N_ini+1 : end-1, 1),'Color', [0.87 0.40 0.69],'linewidth',plt_set.thick), 
xlim([0 49]);
str = str_xaxis;
x_title = xlabel(str,'Interpreter','latex');
str_y = '$y_t$';
...
if sub_num>0
    ...
    fig_BPI_single = plotting_BPI_single_h(BPI_inst_single, plt_set, 13);
end
