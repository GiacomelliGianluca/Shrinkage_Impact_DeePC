function fig_H_map = Hankel_map_PWA(str_cl_type, M, H_label, plt_opt, fig_index, string_matr_type)
...
if strcmp(str_cl_type,'cl')
    ...
    if M == 2 % Two modes
        plt_m1 = scatter(col(label == 1), raws(label == 1), sz, [0.10196078431372549 0.5019607843137255 0.7333333333333333],'filled', 'DisplayName', ['Mode ', num2str(modes(1))]); % data mode_1
        plt_m2 = scatter(col(label == 2), raws(label == 2), sz, [0.6274509803921569 0 0],'filled', 'DisplayName', ['Mode ', num2str(modes(2))]);  % data mode_2
        plt_unc = scatter(col(label == -1), raws(label == -1), sz, [0 0 0],'filled', 'DisplayName', 'Unclustered data');  % Unclustered data
        legend([plt_m1, plt_m2, plt_unc], 'Location','best');
    else % One mode
        plt_m1 = scatter(col(label == 1), raws(label == 1), sz, [0.10196078431372549 0.5019607843137255 0.7333333333333333],'filled', 'DisplayName', ['Mode ', num2str(modes(1))]); % data mode_1
        plt_unc = scatter(col(label == -1), raws(label == -1), sz, [0 0 0],'filled', 'DisplayName', 'Unclustered data');  % Unclustered data
        legend([plt_m1, plt_unc], 'Location','best');
    end
end
