function [J_all, J_tr, J_g, g_opt, u_opt_seq, y_opt_seq] = CAP_DeePC_Mos_y_f_lambda_g(u_ini, y_ini, u_r, y_r, n_u, n_y, Q, R, N_h, N_ini, N_ex, H_u, H_y, D, n_mos, lambda_g, ind_H, num_sub, slc_norm)
...
% FHCOP_rate
    ...
    % - n_mos: number of hankel matrix in the Mosaic Hankel
    ...
    cvx_begin quiet
        ...
        CAP_i = cell(n_mos,1);
        for i = 1 : n_mos
            Car(i) = Car(i) + length(g(ind_H(i,1) : ind_H(i,2)));
            CAP_i{i,1} = [CAP_i{i,1}; g(ind_H(i,1) : ind_H(i,2))];
